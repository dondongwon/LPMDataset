...
class PIENet(nn.Module):
  ...
  def forward(self, out, x, pad_mask=None):
    ...
    out = self.layer_norm(out + residual.squeeze())
...
class EncoderImage(nn.Module):
  ...
  def __init__(self, opt):
    ...
    embed_size, self.num_embeds = opt.embed_size, opt.num_embeds
    ...
    if self.use_attention:
      self.pie_net = PIENet(self.num_embeds, cnn_dim, embed_size, cnn_dim//2, opt.dropout)
